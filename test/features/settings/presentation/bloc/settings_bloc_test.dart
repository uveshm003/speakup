...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockSettingsRepository.getSettings()).thenAnswer((Invocation _) async => const Right<Failure, UserSettings>(UserSettings()));
  });

  SettingsBloc buildBloc() {
    return SettingsBloc(settingsRepository: mockSettingsRepository, sessionRepository: mockSessionRepository, historyBloc: mockHistoryBloc);
  }
  ...
  group('SettingsBloc', () {
    ...
    blocTest<SettingsBloc, SettingsState>(
      ...
      build: () {
        when(() => mockSettingsRepository.getSettings()).thenAnswer((Invocation _) async => const Right<Failure, UserSettings>(UserSettings()));
        when(() => mockSettingsRepository.saveSettings(any())).thenAnswer((Invocation _) async => const Right<Failure, void>(null));
        return buildBloc();
      },
      setUp: () {
      ...
      expect: () => <SettingsState>[const SettingsState(status: SettingsStatus.success, settings: UserSettings(defaultTimerSeconds: 90))],
      verify: (SettingsBloc bloc) {
    ...
    blocTest<SettingsBloc, SettingsState>(
      ...
      build: () {
        when(() => mockSessionRepository.clearAllSessions()).thenAnswer((Invocation _) async => const Right<Failure, void>(null));
        when(() => mockSettingsRepository.updateStreak()).thenAnswer((Invocation _) async => const Right<Failure, void>(null));
        when(() => mockSettingsRepository.getSettings()).thenAnswer((Invocation _) async => const Right<Failure, UserSettings>(UserSettings()));
        return buildBloc();
      },
      skip: 2,
