...
class SessionRepositoryImpl implements SessionRepository {
  ...
  Future<Either<Failure, List<PracticeSession>>> getAllSessions() async {
    try {
      ...
      list.sort((PracticeSessionEntity a, PracticeSessionEntity b) =>
          b.completedAt.compareTo(a.completedAt));
      return Right<Failure, List<PracticeSession>>(
        list.map(practiceSessionFromEntity).toList(),
      );
    } catch (e, _) {
  ...
  Future<Either<Failure, List<PracticeSession>>> getSessionsByDateRange({
    ...
    try {
      ...
      final List<PracticeSessionEntity> filtered = all.where(
        (PracticeSessionEntity e) {
          final DateTime t = e.completedAt;
          return !t.isBefore(start) && !t.isAfter(end);
        },
      ).toList()
        ..sort((PracticeSessionEntity a, PracticeSessionEntity b) =>
            b.completedAt.compareTo(a.completedAt));
      return Right<Failure, List<PracticeSession>>(
        filtered.map(practiceSessionFromEntity).toList(),
      );
    } catch (e, _) {
      return Left<Failure, List<PracticeSession>>(CacheFailure(e.toString()));
    }
  ...
  Future<Either<Failure, void>> deleteSession(String sessionId) async {
    try {
      final Query<PracticeSessionEntity> q = _box
          .query(PracticeSessionEntity_.sessionId.equals(sessionId))
          .build();
      try {
        final PracticeSessionEntity? e = q.findFirst();
        if (e == null) {
          return Left<Failure, void>(
            CacheFailure('Session not found: $sessionId'),
          );
        }
        _box.remove(e.id);
        return const Right<Failure, void>(null);
