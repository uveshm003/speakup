...
class FavoritesBloc extends Bloc<FavoritesEvent, FavoritesState> {
  FavoritesBloc({required CardRepository cardRepository})
      ...
      (failure) async {
        emit(
          state.copyWith(
            ...
            errorMessage: failure.message ?? 'Could not load favorites',
          ),
        );
      },
  ...
  Future<void> _onRemove(
    ...
    await result.fold(
      (failure) async {
        emit(
          state.copyWith(
            errorMessage: failure.message ?? 'Could not update favorite',
          ),
        );
      },
      (_) async {
  ...
  void _onDrawRequested(
    FavoriteDrawRequested event,
    Emitter<FavoritesState> emit,
  ) {
