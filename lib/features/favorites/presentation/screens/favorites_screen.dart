...
class _FavoritesScreenState extends State<FavoritesScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _fabPulseCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 480));
    _fabScale = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: _fabPulseCtrl, curve: Curves.elasticOut));

    _gridFadeCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 380));
          ...
          body: SafeArea(
            child: CustomScrollView(
              slivers: <Widget>[
                // ── Header ─────────────────────────────────────────────────
                SliverPadding(
                  padding: pagePad.copyWith(top: pagePad.top + AppSpacing.md, bottom: 0),
                ...
                if (state.cards.isNotEmpty)
                  SliverPadding(
                    padding: EdgeInsets.only(top: AppSpacing.lg, bottom: AppSpacing.sm, left: pagePad.left, right: pagePad.right),
                    sliver: SliverToBoxAdapter(
                      child: AnimatedOpacity(
                        ...
                        child: AnimatedSlide(
                          ...
                          offset: _showFilters ? Offset.zero : const Offset(0, 0.08),
                          child: _FilterRow(
                            cards: state.cards,
                            selectedCategory: _selectedCategory,
                            selectedDifficulty: _selectedDifficulty,
                            onCategoryChanged: (String? v) => setState(() => _selectedCategory = v),
                            onDifficultyChanged: (Difficulty? v) => setState(() => _selectedDifficulty = v),
                          ),
                        ),
                      ),
                    ),
                ...
                if (state.status == FavoritesStatus.loading && state.cards.isEmpty)
                  SliverPadding(
                    ...
                    sliver: SliverToBoxAdapter(
                      child: ShimmerGridPlaceholder(crossAxisCount: cross, itemCount: cross * 3),
                    ),
                  )
                // ── Error ──────────────────────────────────────────────────
                ...
                // ── Empty state ────────────────────────────────────────────
                  ...
                  SliverFillRemaining(
                    child: AnimatedOpacity(
                      ...
                      child: _EmptyFavorites(onBrowse: () => ctx.go(AppRoutes.home)),
                    ),
                  )
                  ...
                  SliverFillRemaining(
                    child: Center(
                      child: Column(
                        ...
                        children: <Widget>[
                          Icon(Icons.filter_list_off_rounded, size: 48, color: theme.colorScheme.onSurfaceVariant.withValues(alpha: 0.45)),
                ...
                // ── Favorites grid ─────────────────────────────────────────
                  ...
                  SliverPadding(
                    padding: pagePad.copyWith(top: AppSpacing.md, bottom: AppSpacing.huge + 16),
          ...
          floatingActionButton: state.cards.isNotEmpty
              ? AnimatedBuilder(
                  ...
                  child: FloatingActionButton(
                    ...
                    onPressed: () => _drawRandomFrom(context, filtered.isEmpty ? state.cards : filtered),
      ...
      children: <Widget>[
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              Text('Favorites', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700, letterSpacing: -0.3)),
              const SizedBox(height: AppSpacing.xs),
              Text(
                count == 0 ? 'Your saved cards will appear here' : '$count ${count == 1 ? 'card' : 'cards'} saved',
                style: theme.textTheme.bodyMedium?.copyWith(color: theme.colorScheme.onSurfaceVariant),
              ),
            ],
          ),
        ...
        Container(
          ...
          child: Icon(Icons.favorite_rounded, size: 22, color: theme.colorScheme.error),
        ),
      ],
    );
  }
...
class _FilterRow extends StatelessWidget {
  ...
  bool get _hasActiveFilter => selectedCategory != null || selectedDifficulty != null;
  ...
  Widget build(BuildContext context) {
    ...
    final List<String> categories = cards.map((TopicCard c) => c.category).toSet().toList()..sort();
                ...
                return Padding(
                  ...
                  child: _FilterChip(
                    ...
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: <Widget>[
                        Text(emojiForCategory(cat), style: const TextStyle(fontSize: 12)),
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        Container(
                          width: 7,
                          height: 7,
                          decoration: BoxDecoration(shape: BoxShape.circle, color: sel ? diffColor : diffColor.withValues(alpha: 0.5)),
                        ),
                        const SizedBox(width: AppSpacing.xs),
                        Text(
                          _diffLabel(d),
                          style: theme.textTheme.labelSmall?.copyWith(
                            fontWeight: FontWeight.w600,
                            color: sel ? diffColor : theme.colorScheme.onSurfaceVariant,
                          ),
                        ),
                      ],
                    ),
...
class _FilterChip extends StatelessWidget {
  const _FilterChip({required this.onTap, required this.selected, required this.activeColor, required this.child});
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected ? activeColor.withValues(alpha: 0.14) : theme.colorScheme.surfaceContainerHighest.withValues(alpha: 0.6),
          borderRadius: BorderRadius.circular(AppRadius.full),
          border: Border.all(
            width: selected ? 1.4 : 1.0,
            color: selected ? activeColor.withValues(alpha: 0.55) : theme.colorScheme.outlineVariant.withValues(alpha: 0.4),
          ),
        ),
        child: child,
      ),
          ...
          children: <Widget>[
            ScaleTransition(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(shape: BoxShape.circle, color: theme.colorScheme.errorContainer.withValues(alpha: 0.35)),
                child: Icon(Icons.favorite_border_rounded, size: 52, color: theme.colorScheme.error.withValues(alpha: 0.7)),
              ),
            ),
        ...
        child: InkWell(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Container(
                height: 4,
                decoration: BoxDecoration(gradient: LinearGradient(colors: <Color>[accent, accent.withValues(alpha: 0.45)])),
              ),
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(color: accent.withValues(alpha: 0.13), borderRadius: BorderRadius.circular(AppRadius.full)),
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: <Widget>[
                      ...
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: AppSpacing.sm, vertical: 2),
                        decoration: BoxDecoration(
                          color: _difficultyColor(context, card.difficulty).withValues(alpha: 0.12),
                          borderRadius: BorderRadius.circular(AppRadius.xs),
                        ),
                        child: Text(
                          _difficultyLabel(card.difficulty),
                          style: theme.textTheme.labelSmall?.copyWith(fontWeight: FontWeight.w700, color: _difficultyColor(context, card.difficulty)),
                        ),
                      ),
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                ListTile(
                  leading: Container(
                    padding: const EdgeInsets.all(AppSpacing.sm - 2),
                    decoration: BoxDecoration(color: accent.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(AppRadius.sm)),
                    child: Icon(Icons.visibility_rounded, color: accent, size: 20),
                  ),
                ...
                ListTile(
                  leading: Container(
                    ...
                    child: Icon(Icons.heart_broken_rounded, color: theme.colorScheme.error, size: 20),
                  ),
