            ...
            appBar: AppBar(
              title: Row(
                children: <Widget>[
                  ...
                  Expanded(child: Text(widget.category.name, maxLines: 1, overflow: TextOverflow.ellipsis)),
                ],
              ),
              actions: <Widget>[
                TextButton.icon(
                  onPressed: () async {
                    final Object? result = await context.push(AppRoutes.createCard, extra: CreateCardRouteArgs(category: widget.category));
                              ...
                              subtitle: Padding(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: _difficultyColor(card.difficulty, theme).withValues(alpha: 0.15),
                                    borderRadius: BorderRadius.circular(AppRadius.sm),
                                  ),
                                  child: Text(
                                    _difficultyLabel(card.difficulty),
                                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: _difficultyColor(card.difficulty, theme)),
                                  ),
                                ),
                              ),
                              trailing: Row(
                                ...
                                children: <Widget>[
                                  IconButton(
                                    ...
                                    onPressed: () async {
                                      final Object? result = await context.push(
                                        ...
                                        extra: CreateCardRouteArgs(category: widget.category, existingCard: card),
                                      );
                                      if (result == true && context.mounted) {
                                        context.read<CustomCardBloc>().add(CardsLoadRequested(widget.category.categoryId));
                                      }
                                  ...
                                  IconButton(icon: const Icon(Icons.delete_outline, size: 20), onPressed: () => _deleteCard(card)),
                                ],
                              ),
                            ),
                          ),
            ...
            bottomNavigationBar: state.cards.isEmpty
                ...
                : SafeArea(
                    child: Padding(
                      ...
                      child: FilledButton(
                        onPressed: () {
                          context.push('${AppRoutes.cardDraw}?category=${Uri.encodeComponent(drawParam)}');
                        },
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            Icon(Icons.style_outlined, size: 72, color: theme.colorScheme.primary.withValues(alpha: 0.55)),
            const SizedBox(height: AppSpacing.lg),
            Text('No cards yet', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
            ...
            FilledButton(onPressed: onAdd, child: const Text('Add Card')),
          ],
        ),
      ),
    );
